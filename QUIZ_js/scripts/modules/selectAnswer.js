const selectAnswer = (e, questions, state) => {
  ...
  if (questions.length > state.index + 1) {
    ...
    if (state.score === questions.length) {
      resultElement.innerHTML = `
        ...
        `;
    } else if (state.score < questions.length / 2) {
      resultElement.innerHTML = `
        <h1><i class="fas fa-sad-tear"></i></h1>
        <h3>You really need to improve your CSS skills! You scored ${state.score} from ${questions.length} questions!</h3>
        `;
    } else {
      resultElement.innerHTML = `
          <h1><i class="fas fa-rocket"></i></h1>
          <h3>Nice job! Your score is ${state.score} from ${questions.length}. Don't stop learning!</h3>
          `;
    }
  }
};
